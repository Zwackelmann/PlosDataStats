...
def tweetsBetweenDay(documents, lowerBound, upperBound):
    return [[tweet.text, tweet.timestamp, tweet.username,doc.doi, doc.publicationTimestamp] for doc in documents for tweet in doc.tweets 
        if 
            ((lowerBound*60*60*24) <= (tweet.timestamp - doc.publicationTimestamp) <= (upperBound*60*60*24)) and
            canBeEncoded(tweet.text)
    ]
...
tweetTexts = map(lambda tweetdata: "\t".join([tweetdata[0], str(tweetdata[1]), tweetdata[2], tweetdata[3], str(tweetdata[4])]), tweets)
