...
from main.util.common import SimpleDoc, powerset, Log, User
import math
import itertools

userListScore = json.load(open("data/userListExpertScores.json"))
...
for ind, metricName in zip(range(0, len(metricNames)), metricNames):
    ...
    for doc in docs:
        tweetScore = 0
        for user in doc[0]:
            userObj = User.findByTwitterName(user)
            if userObj != None and userObj.numFollowers != None:
                tweetScore += userObj.numFollowers
                count2 += 1
            else:
                count += 1
